...
bool FPManager::checkTesseraInCanvas(Cord lowerLeft, len_t width, len_t height) const {
...
bool FPManager::checkTileInCanvas(Tile &tile) const{
    bool x_valid, y_valid;
...
len_t FPManager::getCanvasHeight() const {
    return this->mCanvasHeight;
}

// void FPManager::translateGlobalFloorplanning(const pp::GlobalSolver &solver) {
...
void FPManager::translateGlobalFloorplanning(const rg::GlobalSolver &solver) {
    // You could define the I/O of this function
    // To create a soft Tessera:
    // Tessera *newTess = new Tessera(tesseraType::SOFT, "Name", 456, Cord(4,5), 3, 4);
    // softTesserae.push_back(newTess);
...
void FPManager::detectfloorplanningOverlaps() {
...
void FPManager::splitTesseraeOverlaps(){
...
Tile *FPManager::findPoint(const Cord &key) const{
    assert(key >= Cord(0, 0));
...
Tile *FPManager::findPoint(const Cord &key, Tile *initTile) const{
    assert(key >= Cord(0, 0));
    assert(key.x < getCanvasWidth());
    assert(key.y < getCanvasHeight());
...
void FPManager::findTopNeighbors(Tile *centre, std::vector<Tile *> &neighbors) const{
...
void FPManager::findDownNeighbors(Tile *centre, std::vector<Tile *> &neighbors) const{
...
void FPManager::findLeftNeighbors(Tile *centre, std::vector<Tile *> &neighbors) const{
...
void FPManager::findRightNeighbors(Tile *centre, std::vector<Tile *> &neighbors) const{
...
void FPManager::findAllNeighbors(Tile *centre, std::vector<Tile *> &neighbors) const{
...
bool FPManager::searchArea(Cord lowerleft, len_t width, len_t height, Tile &target) const{
...
bool FPManager::searchArea(Cord lowerleft, len_t width, len_t height) const{
    len_t searchRBorderHeight = lowerleft.y + height - 1;
    Tile *currentFind = findPoint(Cord(lowerleft.x, searchRBorderHeight));
...
void FPManager::enumerateDirectArea(Cord lowerleft, len_t width, len_t height, std::vector <Tile *> &allTiles) const{
    len_t searchRBorderHeight = lowerleft.y + height - 1;
...
void FPManager::enumerateDirectAreaRProcess(Cord lowerleft, len_t width, len_t height, std::vector <Tile *> &allTiles, Tile *targetTile) const{
...
void FPManager::insertFirstTile(Tile &newTile){
...
void FPManager::insertTile(Tile &tile){
...
void FPManager::visualiseArtpiece(const std::string outputFileName, bool checkBlankTile) {
...
void FPManager::traverseBlank(std::ofstream &ofs,  Tile &t, std::vector <Cord> &record) {
...
void FPManager::visualiseAddMark(Tile * markTile){
    this->mMarkedTiles.push_back(markTile);
}

void FPManager::visualiseRemoveAllmark(){
...
void FPManager::visualiseDebug(const std::string outputFileName){
...
void FPManager::visualiseDebugDFS(std::ofstream &ofs, Tile &t, std::vector <Cord> &record){
...
void FPManager::combineVerticalMergeableBlanks(Tile *upTile, Tile *downTile){
...
bool FPManager::searchTesseraeIncludeTile(Tile *tile, std::vector <Tessera *> &inTessera) const {
...
void FPManager::collectAllTilesDFS(Tile &head, std::vector <Cord> &record, std::vector<Tile *> &allTiles) const{
...
double calculateHPWL(FPManager *legaliser, const std::vector<RectGrad::ConnStruct> &connections, bool printReport){
    double HPWL = 0;
...
void outputFinalAnswer(FPManager *legaliser, const RectGrad::Parser &rgparser, const std::string outputFileName){
